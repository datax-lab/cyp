...
class CYPModel(nn.Module):
    ...
    def forward(self,smile_compound_data, smile_adjacency_data, protein_data):
        ...
        #print ("w1_shape:",w1.shape)
        #w1 = self.bc1(w1)
        w_1 = F.avg_pool2d(w1, kernel_size=w1.size()[-2:]).view(-1,)
        #print ("w_1_shape:",w_1.shape)
        u = w_1.repeat(w1.shape[-2:]).view(w1.shape[0],w1.shape[1], w1.shape[2], w1.shape[-1])
        #print ("u_shape:",u.shape)
        wu_vec = (u-w1)**2
        #print ("wu_vec_shape:",wu_vec.shape)
        wu = F.avg_pool2d(wu_vec, kernel_size=wu_vec.size()[-2:]).view(-1,) 
        #print ("wu_shape:",wu.shape)
        w1 = torch.cat((w_1, wu), 0).view(w1.shape[0], -1)
        #print ("w1_shape:",w1.shape)
        ...
        #print ("w2_shape:",w2.shape)
        #w1 = self.bc1(w1)
        w_2 = F.avg_pool2d(w2, kernel_size=w2.size()[-2:]).view(-1,)
        #print ("w_2_shape:",w_2.shape)
        #print (w1.shape)
        u = w_2.repeat(w2.shape[-2:]).view(w2.shape[0],w2.shape[1], w2.shape[2], w2.shape[-1])
        #print ("u_shape:",u.shape)
        wu_vec = (u-w2)**2
        #print ("wu_vec_shape:",wu_vec.shape)
        wu = F.avg_pool2d(wu_vec, kernel_size=wu_vec.size()[-2:]).view(-1,)
        #print ("wu_shape:",wu.shape)
        w2 = torch.cat((w_2, wu), 0).view(w2.shape[0], -1)
        ...
        #print ("w3_shape:",w3.shape)
        #w1 = self.bc1(w1)
        w_3 = F.avg_pool2d(w3, kernel_size=w3.size()[-2:]).view(-1,)
        #print ("w_3_shape:",w_3.shape)
        #print (w1.shape)
        u = w_3.repeat(w3.shape[-2:]).view(w3.shape[0],w3.shape[1], w3.shape[2], w3.shape[-1])
        #print ("u_shape:",u.shape)
        wu_vec = (u-w3)**2
        #print ("wu_vec_shape:",wu_vec.shape)
        wu = F.avg_pool2d(wu_vec, kernel_size=wu_vec.size()[-2:]).view(-1,)
        #print ("wu_shape:",wu.shape)
        w3 = torch.cat((w_3, wu), 0).view(w3.shape[0], -1)
        ...
        #print ("w4_shape:",w4.shape)
        #w1 = self.bc1(w1)
        w_4 = F.avg_pool2d(w4, kernel_size=w4.size()[-2:]).view(-1,)
        #print ("w_4_shape:",w_4.shape)
        #print (w1.shape)
        u = w_4.repeat(w4.shape[-2:]).view(w4.shape[0],w4.shape[1], w4.shape[2],  w4.shape[-1])
        #print ("u_shape:",u.shape)
        wu_vec = (u-w4)**2
        #print ("wu_vec_shape:",wu_vec.shape)
        wu = F.avg_pool2d(wu_vec, kernel_size=wu_vec.size()[-2:]).view(-1,)
        #print ("wu_shape:",wu.shape)
        w4 = torch.cat((w_4, wu), 0).view(w4.shape[0], -1)
        #print ("w4 shape:",w4.shape)

        #w4 = F.avg_pool1d(w4, kernel_size=w4.size()[-1]).view(-1, 1)

        #print (w1.shape, w2.shape)
        word_vectors = torch.cat((w1, w2, w3, w4), 1).view(w1.shape[0], -1)
