...
COMPOUND_DATA_PATH = "/home/yaganapu/CYP/cyp_update/benchmarks/bacteria_data/transformercpi/validation/new_padded_compounds.npy"
ADJACENCY_DATA_PATH = "/home/yaganapu/CYP/cyp_update/benchmarks/bacteria_data/transformercpi/validation/new_padded_adjacencies.npy"
PROTEIN_DATA_PATH = "/home/yaganapu/CYP/cyp_update/benchmarks/bacteria_data/transformercpi/validation/new_padded_proteins.npy"
TARGET_DATA_PATH = "/home/yaganapu/CYP/cyp_update/benchmarks/bacteria_data/transformercpi/validation/interactions.npy"


#--------------------------------Attempt_* FOLDER_CREATION--------------------------------#
DIR_NAME = BASE_PATH + '/transformercpi/' + '10bins_bce_Attempt_' + str(0) + '/'
...
# # Complete Results
# complete_results = pd.read_csv('/home/yaganapu/CYP/cyp_update/benchmarks/phase2/transformer_data_for_phase2_30bins.csv')
...
#     print("Class Weights: ", class_weights)
    ...
    smileTestData_compound = tt_smile_compound_data.cuda()
    smileTestData_adjacency = tt_smile_adjacency_data.cuda()
    
    proteinTestData = tt_protein_data.cuda()
    
    Test_ori_Targets = targets.cuda()
    ...
    model.load_state_dict(torch.load("/home/yaganapu/CYP/cyp_update/benchmarks/phase2/transformercpi/30bins_cl_Attempt_0/my_model_cl0.pth"))
    model.to(device)
...
#             print("Epoch#", epoch, "Loss=", loss_o,"MSE=",msle_l,"BCE =",bce_l)
...
#     torch.save(model.state_dict(), DIR_NAME + '/my_model_bce' + str(m) + '.pth')
    ...
    with torch.no_grad():
        ...
        test_predictions = model(smileTestData_compound,smileTestData_adjacency, proteinTestData).cpu()
    ...
    y_test_pred = [1 if i > 0.5 else 0 for i in test_predictions]
    y_test_true = [i for i in Test_ori_Targets]
    print("Test Accuracy: ", round((sum([1 if i==j else 0 for i, j in zip(y_test_pred, y_test_true)])/len(y_test_true)),4))
    test_accuracy_final.append(sum([1 if i==j else 0 for i, j in zip(y_test_pred, y_test_true)])/len(y_test_true))

    #train = train_predictions
    
    #train_ = train.numpy()
    
#     train_ = test_predictions.numpy()
...
#np.save(DIR_NAME+"test_accuracy_on_val",np.array(test_accuracy_final))
